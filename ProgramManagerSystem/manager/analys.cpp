    ...
    ui(new Ui::Analys)
{
    ...
    model->setHeaderData(4,Qt::Horizontal,QString::fromStdString("工资"));
    ...
    {
        sum += Resources::m_worker[i]->salary;
        ...
        model->setItem(i,4,new QStandardItem(QString::fromStdString(Resources::m_worker[i]->m_salary)));
    ...
    if(ui->radioButton_department->isChecked())
    {
        ...
        for(int i = 0;i < (int)Resources::m_worker.size();i++)
        {
            if(Resources::m_worker[i]->m_department == inf)
            {
                ...
                model->setItem(whichline,4,new QStandardItem(QString::fromStdString(Resources::m_worker[i]->m_salary)));
                model->setItem(whichline++,5,new QStandardItem(QString::fromStdString(Resources::m_worker[i]->m_adress)));
                sum += Resources::m_worker[i]->salary;
            }
        }
