...
void ManagerDialog::on_pushButton_back_clicked()
{
    ...
    else
    {
        ...
        for (int i = 0; i < (int)Resources::m_worker.size(); i++)
        {
            //file << ID << " " << accout << " " << password << " " << name
            //	<< " " << department << " " << position << " " << address;
            file << Resources::m_worker[i]->ID << " " << Resources::m_worker[i]->m_accout << " " << Resources::m_worker[i]->m_salary << " "
...
//各个功能页面的跳转
void ManagerDialog::on_pushButton_Modify_clicked()
{
...
void ManagerDialog::on_pushButton_finance_clicked()
{
